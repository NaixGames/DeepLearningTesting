...
class NNSerializer:
	...
	def load_weights(self, path: str) -> None:
		#TODO: Some checks to make sure dimensions align?
          
		torch.serialization.add_safe_globals([torch.nn.modules.container.ParameterList])
          
		self.neural_network.layers_weights = torch.load(path + "weights.pt")
		self.neural_network.layers_bias = torch.load(path + "bias.pt")
